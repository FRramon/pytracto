	...
	for ses in ses_list:
		...
		for gen_met in tckgen_method:
			...
			for atlas in atlas_list:
				...
				if createSCmatrix:
					bash_command1 = f'tck2connectome –symmetric –zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_sc_dir}/sc_connectivity_matrix.csv –out_assignment {connectome_sc_dir}/sc_assignments.csv -force'
					print(f"Running command: {bash_command1}")
					subprocess.run(bash_command1,shell = True)

					bash_command2 = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_sc_dir}/sc_assignments.csv {connectome_sc_dir}/exemplar_sc –files single –exemplars {parcellation_file} -force'
				...
				if createFAmatrix:
					...
					bash_command3 = f'tcksample {tracto_dir}/tcksift2/sift_tracks.tck {tracto_dir}/fa/fa.mif {connectome_fa_dir}/mean_FA_per_streamline.csv -stat_tck mean -force'
					subprocess.run(bash_command3, shell=True)

				 	# Create FA matrix
					bash_command4 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file}  {connectome_fa_dir}/fa_connectivity_matrix.csv -scale_file {connectome_fa_dir}/mean_FA_per_streamline.csv -stat_edge mean –out_assignment {connectome_fa_dir}/fa_assignments.csv -force'
					subprocess.run(bash_command4,shell = True)

				    # Create streamtubes
					bash_command = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_fa_dir}/fa_assignments.csv {connectome_fa_dir}/exemplar_fa –files single –exemplars {parcellation_file} -force'
					subprocess.run(bash_command,shell = True)
				...
				if createRDmatrix:
					...
					bash_command3 = f'tcksample {tracto_dir}/tcksift2/sift_tracks.tck {tracto_dir}/rd/rd.mif {connectome_rd_dir}/mean_RD_per_streamline.csv -stat_tck mean -force'
					subprocess.run(bash_command3, shell=True)

				 	# Create RD matrix
					bash_command4 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_rd_dir}/rd_connectivity_matrix.csv -scale_file {connectome_rd_dir}/mean_RD_per_streamline.csv -stat_edge mean –out_assignment {connectome_rd_dir}/rd_assignments.csv -force'
					subprocess.run(bash_command4,shell = True)

				    # Create streamtubes
					bash_command = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_rd_dir}/rd_assignments.csv {connectome_rd_dir}/exemplar_rd –files single –exemplars {parcellation_file} -force'
					subprocess.run(bash_command,shell = True)
				...
				if createADmatrix:
					...
					bash_command3 = f'tcksample {tracto_dir}/tcksift2/sift_tracks.tck {tracto_dir}/ad/ad.mif {connectome_ad_dir}/mean_AD_per_streamline.csv -stat_tck mean -force'
					subprocess.run(bash_command3, shell=True)

				 	# Create AD matrix
					bash_command4 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_ad_dir}/ad_connectivity_matrix.csv -scale_file {connectome_ad_dir}/mean_AD_per_streamline.csv -stat_edge mean –out_assignment {connectome_ad_dir}/ad_assignments.csv -force'
					subprocess.run(bash_command4,shell = True)

				    # Create AD streamtubes
					bash_command = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_ad_dir}/ad_assignments.csv {connectome_ad_dir}/exemplar_ad –files single –exemplars {parcellation_file} -force'
					subprocess.run(bash_command,shell = True)
				...
				if createADCmatrix:
					...
					subprocess.run(bash_command2, shell=True)
				 	...
				 	# Create mean_AD_per_Streamline
					bash_command3 = f'tcksample {tracto_dir}/tcksift2/sift_tracks.tck {tracto_dir}/adc/adc.mif {connectome_adc_dir}/mean_ADC_per_streamline.csv -stat_tck mean -force'
					subprocess.run(bash_command3, shell=True)

				 	# Create AD matrix
					bash_command4 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_adc_dir}/adc_connectivity_matrix.csv -scale_file {connectome_adc_dir}/mean_ADC_per_streamline.csv -stat_edge mean –out_assignment {connectome_adc_dir}/adc_assignments.csv -force'
					subprocess.run(bash_command4,shell = True)

				    # Create AD streamtubes
					bash_command = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_adc_dir}/adc_assignments.csv {connectome_adc_dir}/exemplar_adc –files single –exemplars {parcellation_file} -force'
					subprocess.run(bash_command,shell = True)
				...
				if createNODDImatrix:
					...
					subprocess.run(bash_command1, shell=True)
				 	...
				 	# Create NDI matrix
					bash_command2 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_ndi_dir}/NDI_connectivity_matrix.csv -scale_file {connectome_ndi_dir}/mean_NDI_per_streamline.csv -stat_edge mean –out_assignment {connectome_ndi_dir}/NDI_assignments.csv -force'
					subprocess.run(bash_command2,shell = True)

				    # Create NDI streamtubes
					bash_command3 = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_ndi_dir}/NDI_assignments.csv {connectome_ndi_dir}/exemplar_NDI –files single –exemplars {parcellation_file} -force'
					subprocess.run(bash_command3,shell = True)
					...
					subprocess.run(bash_command4, shell=True)
				 	...
				 	# Create NDI matrix
					bash_command5 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_odi_dir}/ODI_connectivity_matrix.csv -scale_file {connectome_odi_dir}/mean_ODI_per_streamline.csv -stat_edge mean –out_assignment {connectome_odi_dir}/ODI_assignments.csv -force'
					subprocess.run(bash_command5,shell = True)

				    # Create NDI streamtubes
					bash_command6 = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_odi_dir}/ODI_assignments.csv {connectome_odi_dir}/exemplar_ODI –files single –exemplars {parcellation_file} -force'
					...
					subprocess.run(bash_command7, shell=True)
				 	...
				 	# Create NDI matrix
					bash_command8 = f'tck2connectome -symmetric -zero_diagonal {tracto_dir}/tcksift2/sift_tracks.tck {parcellation_file} {connectome_fwf_dir}/FWF_connectivity_matrix.csv -scale_file {connectome_fwf_dir}/mean_FWF_per_streamline.csv -stat_edge mean –out_assignment {connectome_fwf_dir}/FWF_assignments.csv -force'
					subprocess.run(bash_command8,shell = True)

				    # Create NDI streamtubes
					bash_command9 = f'connectome2tck {tracto_dir}/tcksift2/sift_tracks.tck {connectome_fwf_dir}/FWF_assignments.csv {connectome_fwf_dir}/exemplar_FWF –files single –exemplars {parcellation_file} -force'
