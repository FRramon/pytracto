...
def execute_odd_workflow(
    ...
    **kwargs,
    ...
    # Concatenation
    mrconvertPA = MapNode(
        mrt.MRConvert(), name="mrconvertPA", iterfield=["in_file", "in_bvec", "in_bval"]
    )
    ...
    fs_workflow.config["execution"]["stop_on_first_rerun"] = "True"
    ...
    wf_preproc.config["execution"]["stop_on_first_rerun"] = "True"
    ...
    wf_tractography.config["execution"]["stop_on_first_rerun"] = "True"
    ...
    connectome.config["execution"]["stop_on_first_rerun"] = "True"
    ...
    main_wf.config["execution"]["stop_on_first_rerun"] = "True"
    ...
    main_wf.connect([
        (fs_workflow, datasink, [('fs_reconall.aparc_aseg', 'anat.@aparc_aseg')]),
        (connectome, datasink, [('labelconvert.out_file', 'anat.@parcels')]),
        (connectome, datasink, [('transform_parcels.out_file', 'anat.@tranformed_parcels')]),

        (wf_preproc, datasink, [('denoise.out_file', 'dwi.preproc.@denoise')]),
        (wf_preproc, datasink, [('dwpreproc.out_file', 'dwi.preproc.@dwpreproc')]),
        (wf_preproc, datasink, [('unring.out_file', 'dwi.preproc.@unr')]),
        (wf_preproc, datasink, [('biascorrect.out_file', 'dwi.preproc.@biascorrect')]),
        
        (wf_tractography, datasink, [('gen5tt.out_file', 'anat.@5tt')]),
        (wf_tractography, datasink, [('transform5tt.out_file', 'anat.@t5tt')]),
        (wf_tractography, datasink, [('transformT1.out_file', 'anat.@transT1')]),
        (wf_tractography, datasink, [('gmwmi.out_file', 'anat.@gmwmi')]),
        
        (wf_tractography, datasink, [('tcksift2.out_tracks', 'dwi.tractography.@sift2')]),
        (wf_tractography, datasink, [('tcksift2Det.out_tracks', 'dwi.tractography.@sift2det')])
