...
def clean_tracks_done(source_dir, pipe_name):
    wf_tractography_path = os.path.join(
        source_dir, pipe_name, "main_workflow", "wf_tractography"
    )
    ...
    for dirs in processed_dirs:
        ...
        if os.path.exists(sift_path) and os.path.isfile(
            os.path.join(sift_path, "result_tcksift2.pklz")
        ):
