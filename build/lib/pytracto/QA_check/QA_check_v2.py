...
def execute_qa_check(
    ...
    for g in group:
        for ses in ses_list:
            (
                ...
                result_list_not,
                haveProblem,
            ) = get_ids_by_sessions(source_dir, base_dir, g, ses)
            ...
            for sub in subjects_list:
                ...
                tracto_dir = os.path.join(
                    data_dir, "main_workflow", "wf_tractography", identifier
                )
                connectome_dir = os.path.join(
                    data_dir, "main_workflow", "connectome", identifier
                )
                preproc_dir = os.path.join(
                    data_dir, "main_workflow", "preproc", identifier
                )
                ...
                if os.path.isfile(f"{tracto_dir}/gen5tt/5tt.mif") and os.path.isfile(
                    f"{preproc_dir}/biascorrect/biascorrect.mif"
                ):
                    has_5tt = 1
                ...
                if os.path.isfile(f"{tracto_dir}/gmwmi/gmwmi.mif") and os.path.isfile(
                    f"{preproc_dir}/biascorrect/biascorrect.mif"
                ):
                    has_gmwmi = 1
                ...
                if os.path.isfile(
                    f"{tracto_dir}/transformT1/T1_coreg.mif"
                ) and os.path.isfile(f"{preproc_dir}/biascorrect/biascorrect.mif"):
                    # has_fmap = 1
                ...
                if os.path.isfile(
                    f"{tracto_dir}/tcksift2/sift_tracks.tck"
                ) and os.path.isfile(f"{preproc_dir}/biascorrect/biascorrect.mif"):
                    has_probsift = 1
                ...
                if os.path.isfile(
                    f"{tracto_dir}/tcksift2Det/sift_tracks.tck"
                ) and os.path.isfile(f"{preproc_dir}/biascorrect/biascorrect.mif"):
                    has_detsift = 1
                ...
                row_iter = {
                    ...
                    "rsfmri": has_rsfmri,
                    "fmap": has_fmap,
                    "PE_direction": phase_encoding_dir,
                }
    ...
    df.to_csv(
        "/mnt/POOL_IRM08/CONHECT/irm_inventory_conhect_patients.csv"
    )  ### To change
