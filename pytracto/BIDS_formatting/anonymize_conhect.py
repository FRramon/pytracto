...
def anonymize_conhect(base_dir: str):
    """
    ...
    for g in groups:
        ...
        if not os.path.isfile(os.path.join(rawdata_path, f"equivalence_table_{g}.csv")):
            equivalence_table = pd.DataFrame(
                list(zip(subjects_old, subjects_new)),
                columns=["conhect_label", "sub_number"],
            )
            equivalence_table.to_csv(
                os.path.join(rawdata_path, f"equivalence_table_{g}.csv"), index=False
            )
