...
from pytracto.BIDS_formatting.anonymize_conhect import *
from pytracto.BIDS_formatting.heudiconv_parameters import *
...
def execute_bids_formatting(base_dir: str):
    """
    ...
    for g in groups:
        ...
        for i, nsub in enumerate(subjects):
            ...
            for itSes in range(len(sessions)):
                ...
                print(
                    f"Running in group {g} on subject ID {subject_folder}, session ID {session_folder}\n"
                )
                ...
                if DOheudiconv:
                    ...
                    dicom_dir_pattern = (
                        "/base/data/Patients/sub-{subject}/ses-{session}/01-RawData/*/*"
                    )
                ...
                if hasDWI and DOrename:
                    ...
                    for f in json_files:
                        ...
                        with open(os.path.join(dwi_path, f)) as user_file:
                            ...
                            initial_dir = (
                                os.path.splitext(f)[0].split("_")[3].split("-")[1]
                            )
                            ...
                            nifti_files = [
                                f for f in files if os.path.splitext(f)[1] == ".gz"
                            ]
                            nifti_file = [
                                f
                                for f in nifti_files
                                if f.split("_")[2].split("-")[1] == dir_nb
                            ][0]

                            bvec_files = [
                                f for f in files if os.path.splitext(f)[1] == ".bvec"
                            ]
                            bvec_file = [
                                f
                                for f in bvec_files
                            ...
                            bval_files = [
                                f for f in files if os.path.splitext(f)[1] == ".bval"
                            ]
                            bval_file = [
                                f
                                for f in bval_files
                            ...
                            if initial_dir == corrected_dir:
                                ...
                                corrected_common_name = (
                                    separator.join(file_attribute[:3])
                                    + "_dir-"
                                    + corrected_dir
                                    + "_dwi"
                                )
                                ...
                                if not dryrun_rename:
                                    os.rename(
                                        os.path.join(dwi_path, f),
                                        os.path.join(dwi_path, corrected_json),
                                    )
                                    os.rename(
                                        os.path.join(dwi_path, nifti_file),
                                        os.path.join(dwi_path, corrected_nifti),
                                    )
                                    os.rename(
                                        os.path.join(dwi_path, bval_file),
                                        os.path.join(dwi_path, corrected_bval),
                                    )
                                    os.rename(
                                        os.path.join(dwi_path, bvec_file),
                                        os.path.join(dwi_path, corrected_bvec),
                                    )
