...
from pytracto.tractography.pipeline_parameters import *
...
def execute_odd_workflow(data_dir: str, base_directory: str, out_dir: str,
                         tckgen_ntracks_param: int, subject_list: list, ses_list: list, **kwargs):
    """
    Workflow for diffusion MRI tractography and connectivity matrixes creation.
    Particularly adapted for subjects with 3PA/1AP or 3AP/1PA
    ...
    **kwargs: keywords argument for specific pipeline parameters
    ...
    # Concatenation
    mrconvertPA = MapNode(mrt.MRConvert(), name="mrconvertPA", iterfield=[
                          'in_file', 'in_bvec', 'in_bval'])
    ...
    fs_workflow.connect(infosource, 'subject_id',
                        fs_reconall, 'subject_id')
    ...
    # not 4D images)
    fslroi = Node(fsl.ExtractROI(roi_file='5tt_vol0.nii.gz',
                                 t_min=0, t_size=1), name='fslroi')
    ...
    transformconvert = Node(
        mrt.TransformFSLConvert(), name='transformconvert')
    ...
    # tckgenDet.inputs.backtrack = tckgen_backtrack_param
    ...
    wf_tractography = Workflow(
        name="wf_tractography", base_dir=base_directory)
    ...
    wf_tractography.connect(convertb02nii, 'out_file',
                            transformconvert, 'in_file')
    wf_tractography.connect(flirt, 'out_matrix_file',
                            transformconvert, 'in_transform')
    wf_tractography.connect(
        convert5tt2nii, 'out_file', transformconvert, 'reference')
    ...
    wf_tractography.connect(tckgenDet, 'out_file',
                            tcksift2Det, 'in_tracks')
    wf_tractography.connect(transform5tt, 'out_file',
                            tcksift2Det, 'act_file')
    ...
    tck2connectome = MapNode(mrt.BuildConnectome(
    ), name='tck2connectome', iterfield=['in_parc'])
    tck2connectome.inputs.zero_diagonal = True
    tck2connectome.inputs.out_file = "connectome.csv"

    tck2connectomeDet = MapNode(mrt.BuildConnectome(
    ), name='tck2connectomeDet', iterfield=['in_parc'])
    ...
    connectome.connect(transform_parcels, 'out_file',
                        tck2connectomeDet, 'in_parc')
    ...
    main_wf.connect(wf_dc, 'mrcatPA.out_file',
                    wf_preproc, 'denoise.in_file')
    main_wf.connect(wf_dc, 'mrconvertAP.out_file',
                    wf_preproc, 'b0AP_extract.in_file')
    main_wf.connect(wf_preproc, 'biascorrect.out_file',
                    wf_tractography, 'brainmask.in_file')
    main_wf.connect(wf_preproc, 'biascorrect.out_file',
                    wf_tractography, 'b0_extract.in_file')
    main_wf.connect(wf_preproc, 'biascorrect.out_file',
                    wf_tractography, 'dwiresponse.in_file')
    main_wf.connect(wf_preproc, 'biascorrect.out_file',
                    wf_tractography, 'dwi2fod.in_file')
    main_wf.connect(wf_dc, 'sf.anat', wf_tractography, 'gen5tt.in_file')
    main_wf.connect(wf_dc, 'sf.anat', wf_tractography,
                        'transformT1.in_files')
    main_wf.connect(fs_workflow, 'fs_reconall.aparc_aseg',
                    connectome, 'labelconvert.in_file')
    main_wf.connect(wf_tractography, 'transformconvert.out_transform',
                    connectome, 'transform_parcels.linear_transform')
    main_wf.connect(wf_tractography, 'tcksift2.out_tracks',
                    connectome, 'tck2connectome.in_file')
    main_wf.connect(wf_tractography, 'tcksift2Det.out_tracks',
                    connectome, 'tck2connectomeDet.in_file')
    ...
    connectome.write_graph(
        graph2use='orig', dotfilename='./graph_connectome.dot')
